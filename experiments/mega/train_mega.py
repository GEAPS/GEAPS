...
from mrl.utils.misc import set_global_seeds
from mrl.modules.vae_gen import VAEPredictor
...
def main(args):
  ...
  print("Run %i envs in parallel" % args.num_envs)
  ...
  if args.ag_curiosity is not None:
    ...
    if args.ag_curiosity == 'minq':
      ...
      config.ag_curiosity = SuccessAchievedGoalCuriosity(max_steps=args.env_max_step, randomize=True, use_qcutoff=use_qcutoff, keep_dg_percent=args.keep_dg_percent)
    elif args.ag_curiosity == 'skewfit':
      config.vae_predictor = VAEPredictor()
      config.ag_curiosity = SkewfitCuriosity(max_steps=args.env_max_step, randomize=True, use_qcutoff=use_qcutoff, keep_dg_percent=args.keep_dg_percent)
    elif args.ag_curiosity == 'rand':
      # only 1 sample is enough as it is inferently uniform.  
      config.ag_curiosity = NonCuriosity(max_steps=args.env_max_step, randomize=True, num_sampled_ags=1, use_qcutoff=use_qcutoff, keep_dg_percent=args.keep_dg_percent)
  ...
  elif args.alg.lower() == 'gcsl':
    config.algorithm = BehaviorClone()
  else:
    raise NotImplementedError
  ...
  if args.ag_curiosity == 'goaldisc':
    ...
    config.vae = PytorchModel("vae", lambda: VAE(e.goal_dim, e.goal_dim))
  ...
  if args.visualize_trained_agent:
    ...
    goexplore_buffer = agent.replay_buffer.buffer.BUFF.buffer_go_explore
    ...
    print("the epoch interval to test the algorithm is", test_interval)
    for epoch in range(int(args.max_steps // args.epoch_len)):
      ...
      if args.save_embeddings:
        ...
        agent.logger.add_embedding('goexplore_buffer', goexplore_buffer.get_batch(last_idxs))
      ...
      if (epoch+1) % 100 == 0:
        agent.save_checkpoint(os.path.join(agent.agent_folder, "checkpoint/epoch%i" % epoch))
...
if __name__ == '__main__':
  ...
  parser.add_argument('--test_interval', default=5, type=int, help="the epoch interval to perform test")
  parser.add_argument('--skill_type', default="maxent", type=str, help="the type of pre-training skills")
  parser.add_argument('--gen_goal', action="store_true", help="generate goals or sample goals from replay buffer. only compatible with Skew-Fit")
  parser.add_argument('--vae_norm_input', action="store_true", help="use normalized input for VAE.")
